...
require_relative './common/app_logging'
...
class Converter
    ...
    activity.each do |key, value|
      ...
      case value
        ...
        when Array
          ...
          @logger.warn("WARN: Unexpected type in activity hash: #{value}")
      end
    end
    ...
    array.each { |value|
      ...
      case value
        ...
        when Array
          ...
          @logger.warn("WARNING: Unexpected type in activity array: #{value}")
      end
    }
    ...
    hash.each { |key, value|
      ...
      case value
        ...
        when Array
          ...
          @logger.warn("WARNING: Unexpected type in activity hash: #{value}")
      end
    }
  ...
  def build_header(keys_template)
    ...
    keys.each { |key|
      begin
        ...
        @logger.error("ERROR, building header: with key: #{key}")
      end
    }
  ...
  def get_data(activity_hash, key)
    ...
    begin
      ...
      @logger.error("ERROR, get_data: removing new lines")
    end
  ...
  def get_collapsed_array_data(activity_hash, key)
    ...
    elsif p "Hmmmm...  double array needs special logic."
    end
    ...
    elsif p "Hmmmm...  double data member needs special logic."
    end
  ...
  def convert_files
    ...
    Dir.glob("#{@config.dir_input}/*.json") do |file|
      ...
      @logger.debug("DEBUG, convert_files: Converting #{File.basename(file)} file...")
      ...
      activities.each { |activity|
        ...
        begin
          ...
          @logger.error("ERROR in convert_files: could not parse activity's JSON.")
        end
    ...
    @logger.info("Finished JSON to CSV conversion. Conversion required #{Time.now - start_time} seconds.")
...
if __FILE__ == $0 #This script code is executed when running this file.
  ...
  convert.get_logger(oConfig.config_path + oConfig.config_name)
